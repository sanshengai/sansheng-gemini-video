...
- 默认使用用户一次性保存的元宝 Web Cookie 在后台完成 `/sph/` 分享链接解析、腾讯 CDN 下载与 Gemini 分析；不启动微信/浏览器、不安装证书、不修改系统代理。
- 新增 `scripts/wechat_auth.py`，Windows 下用当前用户 DPAPI 加密凭证，隐藏输入且不把 Cookie 写入日志、分析 JSON 或仓库。
- `wx_channels_download` 只在用户已自行保持本地服务就绪时作为末级兜底，并修正任务接口为 `/api/task/create_channels`。
- 加入分享链接与媒体域名白名单、失效授权停止策略，以及后台链/凭证回归测试。
