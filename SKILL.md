...
description: 用 Gemini 原生多模态理解本地视频、音频和在线视频链接，联合分析画面、声音与时序；支持 YouTube 免下载直读，并在需要时自动下载 B站、小红书、抖音等链接，或通过本地服务解析/解密微信视频号。用户说“分析视频、看懂视频、录屏质检、视频链接解析、批量验收、逆向视觉音频手法”时使用；只要字幕、只下载不分析或要剪辑出片时不用。
---
...
| 微信视频号 `weixin.qq.com/sph/...` | 本机 `wx_channels_download` API 解析、下载、解密，再分析 | 是 |
...
3. **凭证必须显式** -- 默认不读取浏览器 Cookie；只有用户传 `--cookies` / `--cookies-from-browser` 才使用。配置 `AI_DOUYIN_API_KEY` 视为允许把对应短视频链接发送给该解析服务。
...
python scripts/analyze_video.py <本地绝对路径|在线视频URL>
  ...
  [--download-provider auto|yt-dlp|ai-douyin|wechat-local]
...
- 微信视频号本地服务：`GEMINI_VIDEO_WECHAT_API_BASE`，默认 `http://127.0.0.1:2022`。
...
- `yt-dlp`、解析代理、微信本地服务的错误分别保留，自动路由失败时汇总根因，不把“登录态不足”误报成“不支持平台”。
- 微信视频号媒体可能是加密资源；本地服务未初始化时明确阻断，不尝试把密文送给 Gemini。
...
- 微信视频号可选适配器：调用 `ltaoo/wx_channels_download` 的本地 HTTP API，不复制其受 Commons Clause 附加条件约束的源码。
...
| `scripts/source_resolver.py` | URL 平台识别、下载路由、微信本地服务适配 |
| `scripts/batch_eval.py` | 本地文件夹逐段验收与汇总 |
| `scripts/setup_check.py` | Gemini、ffmpeg、yt-dlp、微信本地服务体检 |
