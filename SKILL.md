...
description: Use when 用户要用 Gemini 原生多模态联合分析本地或在线视频的画面、声音与时序，支持 YouTube 直读，以及按需下载 B站、小红书、抖音或经一次性授权解析微信视频号；触发词：分析视频、录屏质检、链接解析、批量验收。只要字幕、只下载或要剪辑出片时不用此 Skill。
---
